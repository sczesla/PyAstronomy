...
def _displaySummary(scope):
    ...
    for k in six.iterkeys(inventory):
        ...
        maxlenVal = max(maxlenVal, len("%10.5e %s" % (
            getattr(cmod, "f_"+k).magnitude, getattr(cmod, "f_"+k).dimensionality)))
...
class PyAConstants:
    ...
    def constantDetails(self, const):
        ...
        print("Uncertainty           : " +
              str(getattr(self, "f_"+const+"_err")))
    ...
    def __init__(self, fn=None, unitSystem="cgs"):
        if fn is None:
            ...
            fn = os.path.join(os.path.dirname(
                _PyA.__file__), "constants/cdat.dat")
        self.inventory = {}
