...
class AstroTimeLegacyTest(unittest.TestCase):
    ...
    def test_precess(self):
        ...
        for i in smo.range(len(dat[::, 0])):
            ...
            if dat[i, 7] == 0.0:
                rad = False
            r = at.precess(dat[i, 2], dat[i, 3], dat[i, 0],
                           dat[i, 1], FK4=fk4, radian=rad)
    ...
    def test_precess_xyz(self):
        ...
        err = 0
        for i in smo.range(len(dat[::, 0])):
            x, y, z = at.precess_xyz(
                dat[i, 2], dat[i, 3], dat[i, 4], dat[i, 0], dat[i, 1])
    ...
    def test_xyz(self):
        ...
        for i in smo.range(len(dat[::, 0])):
            r = numpy.array(
                at.xyz(dat[i, 0], velocity=True, equinox=dat[i, 7]))
    ...
    def test_aitoff(self):
        ...
        for i in smo.range(len(dat[::, 0])):
            ...
            if (abs((x-dat[i, 2])/x) > 1e-4 and (abs(x) > 1e-2)) or (abs((y-dat[i, 3])/y) > 1e-4 and (abs(y) > 1e-2)):
                print("Problem: ")
                print("  l, b, x(IDL), y(IDL), x(Python), y(Python): ",
                      dat[i, 0], dat[i, 1], x, y, dat[i, 2], dat[i, 3])
                err += 1
        self.assertEqual(err, 0)
...
class IDLTests(unittest.TestCase):
    ...
    def test_sunpos(self):
        ...
        for i in smo.range(len(dat[::, 0])):
            jd, ra, dec, longmed, oblt = pyasl.sunpos(
                dat[i, 0], full_output=True)
    ...
    def test_co_refract_forward(self):
        ...
        for i in smo.range(len(dat[::, 0])):
            r = pyasl.co_refract_forward(
                dat[i, 0], pressure=dat[i, 1], temperature=dat[i, 2]-273.15)
    ...
    def test_eq2hor(self):
        ...
        for i in smo.range(len(dat[::, 0])):
            alt, az, ha = pyasl.eq2hor(
                dat[i, 2], dat[i, 0], dat[i, 1], lon=dat[i, 3], lat=dat[i, 4], alt=dat[i, 5])
            self.assertAlmostEqual(
                alt/dat[i, 6], 1.0, delta=self.p*100., msg="Altitude does not fit " + str(dat[i, ::]))
            self.assertAlmostEqual(
                az/dat[i, 7], 1.0, delta=self.p*100., msg="Azimuth does not fit " + str(dat[i, ::]))
    ...
    def test_mphase(self):
        ...
        for i in smo.range(len(dat[::, 0])):
            ...
            self.assertAlmostEqual(
                f, dat[i, 1], delta=1e-5, msg="Lunar phase does not match.")
            self.assertAlmostEqual(
                f/dat[i, 1], 1.0, delta=1e-5, msg="Lunar phase (relative) does not match")
    ...
    def test_posangle(self):
        ...
        for i in smo.range(len(dat[::, 0])):
            ...
            f = pyasl.positionAngle(
                dat[i, 0]*15.0, dat[i, 1], dat[i, 2]*15.0, dat[i, 3], positive=False)
            self.assertAlmostEqual(
                dat[i, 4], f, delta=1e-9, msg="Position angle does not match.")
...
#    diffs = []
        for i in smo.range(len(dat[::, 0])):
            ...
            corr, hjd = helcorr(-dat[i, 0], dat[i, 1], dat[i, 2],
                                dat[i, 3]*15.0, dat[i, 4], dat[i, 5])
#      print corr, dat[i,6], corr-dat[i,6]
#      diffs.append(corr-dat[i,6])
            self.assertAlmostEqual(
                dat[i, 6], corr, delta=2e-5, msg="Barycentric correction does not match.")
            self.assertAlmostEqual(
                dat[i, 7]+2.4e6, hjd, delta=2e-5, msg="HJD does not match.")
#    plt.hist(diffs)
#    plt.show()
    ...
    def test_airtovac(self):
        ...
        self.assertAlmostEqual(numpy.max(numpy.abs(
            atv-atv2)), 0.0, delta=1e-6, msg="Air to Vac conversion inconsistent.")
        self.assertAlmostEqual(numpy.max(numpy.abs(
            vta-vta2)), 0.0, delta=1e-6, msg="Vac to air conversion inconsistent.")
