...
class ExoplanetsOrg(pp.PyAUpdateCycle):
    ...
    def __init__(self, skipUpdate=False, verbose=False):
        ...
        self._columns[3] = ["pl_msini",
                            "Minimum planetary mass", "MJ", np.float]
        self._columns[4] = ["pl_radj", "Planetary radius", "RJ", np.float]
        self._columns[5] = ["pl_trandep",
                            "Central depth of transit", "(r_p/r_star)^2", np.float]
        self._columns[6] = ["pl_impact",
                            "Impact Parameter", "Stellar Radii", np.float]
        self._columns[7] = ["pl_trandur", "Duration of transit", "d", np.float]
        self._columns[8] = ["pl_tranmid", "Transit midpoint", "BJD", np.float]
        self._columns[9] = ["pl_tperi",
                            "Time of Periastron passage", "d", np.float]
        self._columns[10] = ["pl_orbsmax", "Semi-major-axis", "AU", np.float]
        self._columns[11] = ["pl_orbsmaxr",
                             "Ratio sma to R_star", "Stellar Radii", np.float]
        self._columns[12] = ["pl_orbincl",
                             "Orbital inclination of planet", "deg", np.float]
        self._columns[13] = ["pl_missal",
                             "Orbital misalignment of planet ", "deg", np.float]
        self._columns[14] = ["pl_omega",
                             "Argument of Periastron", "deg", np.float]
        self._columns[15] = [
            "pl_ecc", "Planetary orbital eccentricity", "", np.float]
        self._columns[16] = ["pl_grav",
