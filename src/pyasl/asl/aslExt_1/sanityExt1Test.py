...
#     plt.show()
  ...
  def sanity_ofPoissonPosteriorExample(self):
    """
    Checking sanity of example for Poisson source with BG
    """
    # from __future__ import print_function, division
    import numpy as np
    import matplotlib.pylab as plt
  ...
  def sanity_ofPoissonPosterior(self):
    """
    Checking sanity of Poisson source with BG
    """
    # from __future__ import print_function, division
    import numpy as np
    import matplotlib.pylab as plt
    ...
    f = 0.1
    for nb in [5, 10, 17, 170]:
      for ns in [1, 10, 20]:
        lss = np.arange(0., 3*ns+10, 0.1)
        post = pyasl.ps_pdf_lams(lss, ns, nb, f)
        norm = scinteg.trapz(post, lss)
        self.assertAlmostEqual(norm, 1.0, msg="Poisson Posterior (ns, nb, norm-1. = %3d, %3d, %.5e) not properly normalized." % (ns, nb, (norm-1.)), delta=0.01)
